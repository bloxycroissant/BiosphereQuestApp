...
export const WordScrambleGame: React.FC<Props> = ({ grade, onSuccess, onClose }) => {
  ...
  const playSoundEffect = (type: 'tap' | 'success' | 'timeout') => {
    console.log(`[Word Scramble SFX]: ${type}`);
  };
  ...
  useEffect(() => {
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(starAnim, { toValue: 1, duration: 4000, useNativeDriver: true }),
        Animated.timing(starAnim, { toValue: 0, duration: 4000, useNativeDriver: true }),
      ])
    ).start();
  ...
  const loadNextUniqueChallenge = () => {
    setTimeLeft(30);
    const pool = [
      { id: `w1-${currentGrade}`, prompt: `PLANT (Grows in soil)`, detail: `Grade ${currentGrade} Botany Term 🌿` },
      { id: `w2-${currentGrade}`, prompt: `WATER (Essential liquid)`, detail: `Grade ${currentGrade} Earth Science 💧` },
      { id: `w3-${currentGrade}`, prompt: `ENERGY (Power to do work)`, detail: `Grade ${currentGrade} Physics Concept ⚡` },
    ];
  ...
  return (
    <Animated.View style={[styles.container, { opacity: fadeAnim }]}>
      <Animated.Text style={[styles.floatingStar, { top: '20%', left: '15%', transform: [{ translateY: starAnim.interpolate({ inputRange: [0, 1], outputRange: [0, -15] }) }] }]}>✨</Animated.Text>
      ...
      <View style={styles.contentCard}>
        <Text style={styles.header}>🔤 Word Scramble</Text>
        <Text style={styles.subtext}>{currentChallenge?.detail || 'Unscramble the term!'}</Text>
        ...
        <Animated.View style={{ transform: [{ scale: buttonScale }], width: '100%', alignItems: 'center' }}>
          <TouchableOpacity style={styles.actionButton} onPress={handleNextPress} activeOpacity={0.8}>
            <Text style={styles.actionButtonText}>Next Word ➔</Text>
          </TouchableOpacity>
        </Animated.View>
      </View>
...
const styles = StyleSheet.create({
  ...
  displayText: { color: '#38bdf8', fontSize: 20, fontWeight: '900', textAlign: 'center' },
